...
pma_datasets = pd.read_csv("pma2020_codebook_datasets.csv", encoding='unicode_escape')
pma_datafiles = pd.read_csv("pma2020_codebook_files.csv", encoding='unicode_escape')
...
author_column = [json.dumps(x) for x in author_column]
...
pma_datasets['dv.license'] = "CC BY-NA-SA 4.0"
...
keyword_column = [json.dumps(x) for x in keyword_column]
...
pma_datasets.to_csv("pma2020_codebook_datasets_toupload.csv", index=False)
...
pma_datafiles.to_csv("pma2020_codebook_files_toupload.csv", index=False)